            ...
            if (MyPropertyIdListCount != 0)
            {
                foreach (int MyPropertyId in MyPropertyIdList)
                {
                    ...
                    switch (myPropertyIdString)
                    {
                        ...
                        case "829a":
                            {
                                MyMetadata.ExposureTime.ByteValue = MyImage.GetPropertyItem(MyPropertyId).Value;
                                MyMetadata.ExposureTime.RawValueAsString = BitConverter.ToString(MyMetadata.ExposureTime.ByteValue);
                                string StringValue = "";
